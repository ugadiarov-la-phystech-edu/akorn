...
from source.models.savi import Learned
from source.models.videosaur.model import VideoAkornSAur
from source.models.videosaur.modules.decoders import SlotMixerDecoder
...
from source.models.videosaur.modules.utils import FeatureTimeSimilarity
...
if __name__ == '__main__':
    ...
    slot_attention = SlotAttention(
        inp_dim=args.slot_size, slot_dim=args.slot_size, n_iters=2, use_mlp=False,
    )
    allocator = TransformerEncoder(dim=args.slot_size, n_blocks=2, n_heads=4, memory_dim=args.slot_size,)
    renderer = MLP(inp_dim=args.slot_size, outp_dim=1024, hidden_dims=[1024, 1024, 1024], final_activation=True,)
    decoder = SlotMixerDecoder(
        inp_dim=args.slot_size, outp_dim=n_patches, embed_dim=args.slot_size, n_patches=n_patches, allocator=allocator,
        renderer=renderer, renderer_dim=1024, pos_embed_mode='add', use_layer_norms=True,
    )
    ...
    feature_time_similarity = FeatureTimeSimilarity(temperature=args.similarity_temperature, threshold=0, softmax=True)
    ...
    for epoch in range(args.epochs):
        ...
        for i, batch in train_pbar:
            ...
            optimizer.zero_grad()
            features = output['features_sequence'].flatten(start_dim=1, end_dim=2).transpose(-2, -1)
            features_reconstruction = output['features_reconstruction_sequence'][:, :-1].flatten(start_dim=1, end_dim=2).transpose(-2, -1)
            loss = args.loss_coefficient * torch.nn.functional.cross_entropy(features_reconstruction, features)
            loss.backward()

            grad_norm = torch.nn.utils.clip_grad_norm_(video_akornsaur.parameters(), args.grad_norm_clip)
            optimizer.step()
            scheduler.step()
        ...
        for i, batch in val_pbar:
            ...
            if (i == 0 and do_need_visualize) or do_need_log_ari:
                output = video_akornsaur(images=images, actions=torch.empty((0, batch.shape[1])), prior_slots=None, reconstruct=True, masks=True)
        ...
        if epoch % args.save_every_n_epochs == 0:
            checkpoint = {'model': video_akornsaur.state_dict(), 'optimizer': optimizer.state_dict(),
