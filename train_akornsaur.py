...
def get_loader(data, data_root, data_episode_folder_pattern, data_cache, imsize, batchsize, drop_last=False, num_workers=0, is_eval=False,
               image_file_extension=None):
    from source.data.datasets.objs.load_data import load_data

    dataset, imsize, collate_fn = load_data(data, data_root, data_episode_folder_pattern, data_cache, imsize,
                                            is_eval=is_eval, kind='image', image_file_extension=image_file_extension)
...
if __name__ == '__main__':
    ...
    train_dataloader, _ = get_loader(args.data, args.data_root, args.data_episode_folder_pattern, args.data_cache,
                                     args.model_imsize, args.batchsize, drop_last=True, num_workers=args.num_workers,
                                     is_eval=False, image_file_extension=args.image_file_extension)
    val_dataloader, _ = get_loader(args.data, args.data_root, args.data_episode_folder_pattern, args.data_cache,
                                   args.model_imsize, args.batchsize, drop_last=False, num_workers=args.num_workers,
                                   is_eval=True, image_file_extension=args.image_file_extension)
